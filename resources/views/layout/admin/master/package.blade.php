						...
						<div class="card-body">
							...
							<div class="datatable-dashv1-list custom-datatable-overright">
								...
								<td>	
									...
									<label class="switch">
										 <input type="checkbox" class="toggle-status" data-id="{{ $package->id }}" {{ $package->status == 1 ? 'checked' : '' }}>
                          ...
                          <input type="checkbox"  onclick="return confirm('Are you sure, you want to Change it?')" checked id="togBtn">
                            ...
                            <span class="on">Active</span><!--END--></div>
											...
											@if (session()->get('log_type') == 'Admin')
												...
												@method('DELETE')
												@csrf
												<button type="submit" class="btn btn-warning mx-1" value="{{$package->id}}"><i
														class="fa fa-trash"></i>
												</button>
											</form>
											@endif
...
$(document).on('click','.edit_package', function(e){
    ...
    $.ajax({
        //   data: $('').serialize(),
          url: "{{ url('admin/package') }}/"+pack_id+"/edit",          
          type: "get",
          dataType: 'json',
    ...
    var url ="{{route('package.update', ":updateid")}}";
...
$(document).ready(function() {
    ...
    $(document).on('submit', 'form', function(e) {
        if($(this).find('input[name="_method"][value="DELETE"]').length > 0) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to delete this package! This cannot be undone.",
                icon: 'warning',
