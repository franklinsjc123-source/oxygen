                            ...
                            <span class="on"  value="1" id="active">Active</span><!--END--></div>
										...
										<td><span class="d-flex"> 
											...
											@if (session()->get('log_type') == 'Admin')
												<form
												action="{{ route('designation.master.destroy', $item->id) }}"
												method="post">
												@method('DELETE')
												@csrf
												<button type="submit" class="btn btn-warning mx-1" value="{{$item->id}}"><i
														class="fa fa-trash"></i>
												</button>
											</form>
											@endif
...
$(document).ready(function() {
    ...
    $(document).on('change', '.toggle-status', function() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('designation.master.changestatus') }}",
    ...
    $(document).on('submit', 'form', function(e) {
        if($(this).find('input[name="_method"][value="DELETE"]').length > 0) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to delete this designation! This cannot be undone.",
                icon: 'warning',
...
<script>
    ...
    //    console.log(pin_id);
     $('#exampleModal1').modal('show');
		 ...
		 var url= "{{ route('designation.master.edit', ":id") }}";
	...
	var status = $('#editstatus').val();
			 ...
			 var url= "{{ route('designation.master.update', ":id") }}";
			 url = url.replace(":id", id);
   ...
   // url = url.replace(":updateid", updateid);
    $.ajax({
        //   data: $('').serialize(),    {{ url('admin/zonalupdate') }}/"+id+"/update"
             // url: "{{ url('admin/zonalupdate') }}/"+updateid,
              //console.log(id);
         action: "{{ route('designation.master.store') }}", 
         type: "POST",
