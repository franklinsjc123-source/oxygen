                                        ...
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form class="" method="post" action="{{ route('Departmentssave') }}"
                                                        id="form">
                                                        @csrf
                                                        <div class="form">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($departments as $department)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <label class="switch">
                                                        <input type="checkbox" class="toggle-status" data-id="{{ $department->id }}" {{ $department->status == 1 ? 'checked' : '' }}>
                                                        ...
                                                        <button type="button" class="edit_department btn btn-secondary mx-1" value="{{$department->id}}">
                                                            <i class="fa fa-pencil"></i></button>
                                                                  ...
                                                                  @if (session()->get('log_type') == 'Admin')
																	  <form
                                                                    action="{{ route('departmentdelete', $department->id) }}"
                                                                    method="post">
                                                                    @method('POST')
                                                                    @csrf
                                                                    <button type="submit" class="btn btn-warning mx-1" value="{{$department->id}}"><i
                                                                            class="fa fa-trash"></i>
                                                                    </button>
                                                                </form>
																@endif
...
$(document).ready(function() {
    ...
    $(document).on('change', '.toggle-status', function() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('department.changestatus') }}",
    ...
    $(document).on('submit', 'form', function(e) {
        if($(this).find('input[name="_method"][value="POST"]').length > 0 && $(this).attr('action').includes('departmentdelete')) {
            e.preventDefault();
            var form = this;
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to delete this department! This cannot be undone.",
                icon: 'warning',
    ...
    $.ajax({
        //   data: $('').serialize(),  {{ url('admin/pincode1') }}/"+pin_id+"/edit"
          url: "{{ url('admin/editdepartment') }}/"+id+"/edit",          
          type: "get",
          dataType: 'json',
    ...
    $.ajax({
        //   data: $('').serialize(),    {{ url('admin/zonalupdate') }}/"+id+"/update"
             // url: "{{ url('admin/zonalupdate') }}/"+updateid,
              //console.log(id);
         url: "{{ url('admin/departmentupdate') }}/"+id+"/update", 
         type: "POST",
