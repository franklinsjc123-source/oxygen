                                        ...
                                        <div class="modal-dialog modal-dialog-centered" style="max-width: 550px;" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">												
                                                    <form class="" action="{{route('category-banners.store')}}" method="post" onsubmit="return confirm('Are you sure, you want to Save it?')" enctype="multipart/form-data" >
                                                        @csrf
                                                        <div class="form">
                                    ...
                                    <div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" data-backdrop="false" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered" style="max-width: 550px;" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">												
                                                    <form class="" action="{{route('category-banners.update', 0)}}" method="post" onsubmit="return confirm('Are you sure, you want to Update it?')" enctype="multipart/form-data" >
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="form">
                                            ...
                                            <td>
                                                <span class="d-flex">
                                                  ...
                                                  @if (session()->get('log_type') == 'Admin')
													  <form action="{{ route('category-banners.destroy', $item->id) }}" method="post">
                                                     @method('DELETE')
                                                     @csrf
...
$(document).ready(function() {
    ...
    $(document).on('change', '.toggle-status', function() {
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('category-banners.changestatus') }}",
...
<script>
    $(document).on('click', '.edit_categorybanner', function(e){
        ...
        var url = "{{route('editcategory-banners', ':ad_id')}}";
