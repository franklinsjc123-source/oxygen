										...
										<td>
											<a href="{{ route('staffproduct_colors.edit', $color) }}" class="btn btn-warning" title="Edit"><i class="fa fa-pencil"></i> </a>
											 @if (session()->get('log_type') == 'Admin')
												 <form action="{{ route('staffproduct_colors.destroy', $color) }}"  onsubmit="return confirm('Are you sure you want to delete this color?');" method="POST" style="display:inline;">
												@csrf
												@method('DELETE')
