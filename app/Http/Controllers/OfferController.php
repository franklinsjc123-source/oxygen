...
class OfferController extends Controller
{
	...
	public function store(Request $request, FlasherInterface $flasher)
	{
		...
		if ($request->file('offer_logo')) {
			$offer_logo = $request->file('offer_logo');

			$image =  "offer_logo_" . time() . "_image" . $offer_logo->getClientOriginalExtension();
			$img = Image::make($offer_logo->getRealPath());
			$image_path = "assets/images/offer_logo";
		...
		try {
			...
			$offer_main->title = $request->offertitle;
			$offer_main->type = $request->type;
			$offer_main->offer_logo = $filename;
