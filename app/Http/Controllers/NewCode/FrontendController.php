...
class FrontendController extends Controller
{
    ...
    public function productVar($id = '')
    {
        ...
        $avg = Rating::where('products_id',$prouctsList['id'])->avg('star_rating');
        $percent = $avg > 0 ? ($avg / 5) * 100 : 0;

        $reviewCount = Rating::where('products_id', $prouctsList['id'])->count();

        $myRating = null;
        if (session()->has('customer_id')) {
            $myRating = Rating::where('products_id', $prouctsList['id'])
